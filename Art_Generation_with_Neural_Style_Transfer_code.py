...
def compute_content_cost(content_output, generated_output):
    ...
    # Reshape 'a_C' and 'a_G' to 2D matrices
...
def compute_layer_style_cost(a_S, a_G):
    ...
    # Compute gram matrices for both images S and G
...
def train_step(generated_image):
    with tf.GradientTape() as tape:
        ...
        # Ensure that a_C and a_G have the right shape (with batch dimension)
        if len(a_C[-1].shape) == 4:  # If batch dimension is missing, add it
            a_C_last = tf.expand_dims(a_C[-1], axis=0)
        else:
            a_C_last = a_C[-1]

        if len(a_G[-1].shape) == 4:  # If batch dimension is missing, add it
            a_G_last = tf.expand_dims(a_G[-1], axis=0)
        else:
            a_G_last = a_G[-1]
        
        # Compute the content cost using the activations from block5_conv4
        J_content = compute_content_cost(a_C_last, a_G_last)
