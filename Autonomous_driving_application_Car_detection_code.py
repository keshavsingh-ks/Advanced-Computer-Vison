...
    """
    ...
    # Calculate the union area using the formula: Union(A, B) = A + B - Inter(A, B)
...
def yolo_eval(yolo_outputs, image_shape = (720, 1280), max_boxes=10, score_threshold=.6, iou_threshold=.5):
    ...
    # Use yolo_filter_boxes() to filter with a threshold of score_threshold
    scores, boxes, classes = yolo_filter_boxes(boxes, box_confidence, box_class_probs, score_threshold)
    
    # Scale boxes back to original image shape.
    boxes = scale_boxes(boxes, image_shape)
    
    # Use yolo_non_max_suppression() to perform Non-max suppression with max_boxes and iou_threshold
